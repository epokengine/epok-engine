...
static std::vector<epok::ActorData> epok_hud_entities(const EpokHudNode* nodes,uint32_t count){
    ...
    for(uint32_t i=0;i<count;++i){
        ...
        for(int k=0;k<4;++k){e.image.region[k]=uint16_t(n.region[k]);e.image.borders[k]=uint16_t(n.borders[k]);e.layout_container.padding[k]=epok::Fixed(n.layout_container[3+k],epok::Fixed::RAW);}
    }
    return entities;
}
extern "C" uint32_t epok_hud_compile(const EpokHudNode* nodes,uint32_t count,const int32_t* dimensions,uint32_t textures,int32_t width,int32_t height,const uint32_t* budget,EpokHudCommand* output,uint32_t capacity,uint32_t* stats){
    ...
    compiler.draw(entities.data(),count,first.data(),next.data(),sizes,rects.data());
...
extern "C" uint32_t epok_hud_layout(const EpokHudNode* nodes,uint32_t count,int32_t width,int32_t height,int32_t* out_rects){
    ...
    epok::hud_core::Compiler compiler(sink,width,height,{count,0,0,0});
    compiler.layout(entities.data(),count,first.data(),next.data(),sizes,rects.data());
    for(uint32_t i=0;i<count;++i){out_rects[i*4]=rects[i].x.raw();out_rects[i*4+1]=rects[i].y.raw();out_rects[i*4+2]=rects[i].w.raw();out_rects[i*4+3]=rects[i].h.raw();}
    return count;
}
