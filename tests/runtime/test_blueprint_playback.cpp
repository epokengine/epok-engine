...
static void pending(){assert(tasks.wait_external(7,EntityHandle{0,1},1));}
...
int main(){
    ...
    playback_wait.begin(replacement,33,44);pending();entities[0].active=false;
    director.advance(raw(100),1);director.play(asset,owner,nullptr,1);
    tasks.advance(raw(1),1,true);assert(!poll());entities[0].active=true;assert(!poll());
    ...
    assert(poll()&&playback_wait.result==bp::PlaybackResult::Cancelled);
    h=director.play(asset,owner,nullptr,1);playback_wait.begin(h);pending();entities[0].alive=false;++entities[0].generation;
    director.advance(raw(100),1);observe();tasks.advance(raw(1),1);assert(!poll()&&tasks.size()==0);
    entities[0]=Entity{};playback_wait.begin(h);pending();tasks.advance(raw(1),2);assert(!poll()&&tasks.size()==0);
    director.cancel_all();
    ...
    assert(tasks.wait_external(8,EntityHandle{0,1},1));
    ...
    entities[0].active=true;tasks.advance(raw(1),1);assert(poll());
    unsigned delivered=1;
    assert(tasks.delay(8,raw(5),EntityHandle{0,1},1));
    ...
    for(unsigned i=0;i<3;++i){
        subscription.rearm();assert(tasks.wait_external(8,EntityHandle{0,1},1));observe();
    ...
    subscription.rearm();assert(tasks.wait_external(8,EntityHandle{0,1},1));observe();tasks.advance(raw(1),1);
