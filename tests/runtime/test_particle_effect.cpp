...
int main(){
    ...
    auto h=pool.spawn(asset,world,1,nullptr,DataHandle{0,1});
    test_set_active(0,false);pool.prepare(1);sequences.advance(raw(50),1);pool.advance(raw(50));
    assert(sequences.tick(pool.sequence(h))==0);
    test_set_active(0,true);assert(pool.pause(h,true));pool.prepare(1);sequences.advance(raw(50),1);pool.advance(raw(50));
    assert(sequences.tick(pool.sequence(h))==0);
    ...
    test_invalidate(0);pool.prepare(1);
    ...
    for(unsigned i=0;i<8;++i)assert(sequences.state(sequences.play(empty,DataHandle{1,1},nullptr,1))==timeline::State::Playing);
    ...
    auto tick=[&](int32_t dt){pool.prepare(1);sequences.advance(raw(dt),1);pool.advance(raw(dt));particles.begin(raw(dt));
        particles.emitter(DataHandle{1,1},scene_emitter,world);
