...
/// the class's own methods, in declaration order, because the generated
/// trampolines index the chunk by that slot. Every other function the class
/// inherits follows: a Lua body dispatches to inherited native callables such
/// as `self:damage(...)` through the same `__epok_call` switch, so those need a
/// slot too even though the chunk has no function for them.
///
...
impl Emitter<'_> {
        ...
        match expr {
            ...
            Expr::Read { place, value_type } => match place {
                ...
                _ => {
                    ...
                    self.boundary_out(format!("__epok_getf(self, {slot})"), value_type)
                }
            },
            Expr::Unary {
                op,
    ...
    fn call(
        ...
        let text = self.call_raw(helper, function_id, name, args, span);
        self.boundary_out(text, returns)
    }
            ...
            StatementKind::Evaluate(value) => {
                let rendered = match value {
                    Expr::CallSelf {
                        ...
                        args,
                        ..
                    } => self.call_raw("__epok_call", function_id, name, args, span),
                    Expr::CallParent {
                        function_id,
                        name,
                        args,
...
pub fn package(
    ...
    for (class, ir) in classes {
        ...
        let chunk = match emit_chunk(
            ir,
            &field_slots(&ir.cpp_name, registry),
