...
use crate::{project::Config, scene_dependencies::Input, staging_files::{BuildTicket, Files}};
...
pub fn request(root: &Path, input: &Input, config: &Config, debug: bool) -> Result<String, String> {
    ...
    let editor = EDITOR.get_or_init(|| {
        std::env::current_exe().and_then(std::fs::read)
            .map(|bytes| crate::assets::hash(&bytes)).map_err(|e| e.to_string())
    }).as_ref().map_err(Clone::clone)?;
    Ok(crate::scene_dependencies::hash((
        1u32, editor, input.origin.dependency(root, "play")?,
        crate::scene_dependencies::signature(&input.scene),
        input.editor_override.as_ref().map(|(path, scene)| (path, crate::scene_dependencies::signature(scene))),
        &input.play, &input.play_settings_signature, config, debug,
        crate::project::fingerprint(root)?,
        crate::project::runtime_sources().iter().map(|(path, bytes)| (*path, crate::assets::hash(bytes))).collect::<Vec<_>>(),
    )))
}
impl Receipt {
    pub fn load(build: &Path, request: &str) -> Option<Self> {
        let receipt: Self = crate::document::from_slice(&std::fs::read(build.join(RECEIPT)).ok()?).ok()?;
        if receipt.version != 1 || receipt.request != request { return None; }
    ...
    fn verify_outputs(&self, build: &Path) -> Result<(), String> {
        ...
        for (path, expected) in &self.outputs {
            if !matches!(path.as_str(), "epok.ps-exe" | "epok.elf" | "epok.map" | "epok.bin" | "epok.cue" | "epok.iso") {
                return Err("Invalid cached output path".into());
            }
            if crate::assets::hash(&std::fs::read(build.join(path)).map_err(|e| e.to_string())?) != *expected { return Err(format!("Cached output {path} changed")); }
        }
        if !self.outputs.contains_key("epok.ps-exe") || !self.outputs.contains_key("epok.map") || !self.outputs.contains_key("epok.elf") { return Err("Missing cached outputs".into()); }
        if let Some(disc) = &self.disc {
            if !matches!(disc.as_str(), "epok.cue" | "epok.iso") || !self.outputs.contains_key(disc) { return Err("Missing cached disc".into()); }
            if disc == "epok.cue" && !self.outputs.contains_key("epok.bin") { return Err("Missing cached disc data".into()); }
        }
        Ok(())
    }
    pub fn save(build: &Path, request: String, ticket: BuildTicket, disc: Option<&Path>) -> Result<(), String> {
        ...
        if let Some(name) = disc.as_deref() {
            names.push(name);
            if name == "epok.cue" { names.push("epok.bin"); }
        }
        for name in names {
            outputs.insert(name.into(), crate::assets::hash(&std::fs::read(build.join(name)).map_err(|e| e.to_string())?));
        }
        let receipt = Self { version: 1, request, ticket, outputs, disc };
    ...
    pub fn reuse(self, root: &Path, build: &Path) -> Result<(PathBuf, Option<PathBuf>), String> {
        ...
        if !bytes.starts_with(b"PS-X EXE") || crate::assets::hash(&bytes) != self.outputs["epok.ps-exe"] {
...
mod tests {
    ...
    fn receipt_rejects_changed_missing_and_escaping_outputs() {
        ...
        for name in ["epok.ps-exe", "epok.elf", "epok.map", "epok.cue", "epok.bin"] {
            std::fs::write(build.join(name), b"original").unwrap();
        }
        Receipt::save(&build, "request".into(), ticket, Some(&build.join("epok.cue"))).unwrap();
