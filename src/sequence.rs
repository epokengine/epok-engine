...
impl SourceSelection {
    pub fn validate(&self) -> Result<(), String> {
        ...
        let valid = match self.profile {
            SourceProfile::SonySeqV1 => {
                self.song_id == Some(0) && self.song_index.is_none() && self.record_hash.is_none()
            }
            SourceProfile::SonySepV0 => {
                self.song_id.is_some() && self.song_index.is_none() && self.record_hash.is_none()
            }
            SourceProfile::ConvertedSeqLe32V1 => {
                self.song_id.is_none()
                    && self.song_index.is_some_and(|i| i < 256)
                    && self.record_hash.as_ref().is_some_and(|s| {
                        s.len() == 64
                            && s.bytes()
        ...
        if valid {
            ...
            Err("Invalid source selection: Sony requires song ID; converted SEQ/SEP requires ordinal and lowercase SHA-256 record hash".into())
        }
    }
}
...
pub fn catalog_source(
    ...
    if bytes.starts_with(b"MThd") {
        ...
        return Ok(SourceCatalog {
            ...
            songs: vec![SourceSong {
                ...
                offset: 0,
                length: bytes.len(),
                record_hash: crate::assets::hash(bytes),
                ppqn: ir.ppqn,
    ...
    let profile = match explicit {
        Some(p) => p,
        None => crate::sequence_compat::detect(bytes)?,
    };
    ...
    Ok(SourceCatalog {
        ...
        songs: container
            ...
            .into_iter()
            .enumerate()
            .map(|(ordinal, e)| SourceSong {
                id: e.id,
                ordinal: ordinal as u16,
                offset: e.offset,
                length: e.length,
                record_hash: crate::assets::hash(&bytes[e.offset..e.offset + e.length]),
                ppqn: e.ir.ppqn,
                duration_micros: e.ir.duration_micros,
                events: e.ir.events.len(),
                playback_blockers: e
                    .ir
                    .playback_blockers
                    .iter()
...
pub fn select_source(
    ...
    let song = match profile {
        SourceProfile::ConvertedSeqLe32V1 if song_id.is_none() => {
            catalog.songs.iter().find(|s| Some(s.ordinal) == song_index)
        }
        SourceProfile::SonySeqV1 | SourceProfile::SonySepV0 if song_index.is_none() => {
            catalog.songs.iter().find(|s| Some(s.id) == song_id)
        }
        _ => None,
    }
    ...
    let selection = SourceSelection {
        ...
        song_id,
        song_index,
        record_hash: (profile == SourceProfile::ConvertedSeqLe32V1)
            .then(|| song.record_hash.clone()),
...
pub fn decode_source(
    bytes: &[u8],
    settings: &Settings,
) -> Result<crate::sequence_ir::SequenceIr, String> {
    settings.validate()?;
    let Some(selection) = &settings.source_selection else {
        if bytes.starts_with(b"MThd") {
            return crate::midi::parse_with_profile(bytes, settings.midi_profile);
        }
        let catalog = catalog_source(bytes, None)?;
        return Err(format!(
            "{} contains {} song(s). Select the source profile and song explicitly before import or audition.",
            catalog.profile.map_or("MIDI", |p| p.id()),
            catalog.songs.len()
        ));
    };
    ...
    if selection.profile == SourceProfile::ConvertedSeqLe32V1
        && selection.record_hash.as_deref()
            != Some(
                crate::assets::hash(&bytes[selected.offset..selected.offset + selected.length])
                    .as_str(),
            )
    {
...
pub struct Settings {
    ...
    #[serde(
        default = "legacy_midi_profile",
        skip_serializing_if = "is_legacy_midi_profile"
    )]
...
impl Settings {
    ...
    pub fn validate_playback(&self, ir: &crate::sequence_ir::SequenceIr) -> Result<(), String> {
        self.validate()?;
        if ir.events.iter().any(|event| {
            matches!(
                event.kind,
                crate::sequence_ir::EventKind::Control {
                    controller: 91,
                    value: 1..=127,
                    ..
                }
            )
        }) && crate::psx_music_settings::Recipe::from_settings(self)?.effects
            != crate::psx_music_settings::Effects::Room
        {
        ...
        if let Some(first) = ir.playback_blockers.first() {
            if ir.source_profile.as_deref() == Some("midi-musical-v2") {
                return Err(format!(
                    "MIDI contains {} unsupported events in Musical v2: {} (track {}, tick {}). Review the report and re-export the MIDI with these operations resolved. Ignore unsupported events is a Legacy v1 exception and cannot bypass this profile.",
                    ir.playback_blockers.len(),
                    first.message,
                    first.track + 1,
                    first.tick
                ));
            ...
            return Err(format!(
                "{} playback blocked by {} unresolved source requirements: {}. Ignore unsupported MIDI cannot bypass this incompatibility.",
                ir.source_profile.as_deref().unwrap_or("Sequence"),
                ir.playback_blockers.len(),
                first.message
            ));
        }
...
mod compatibility_tests {
    use super::*;
    fn score(key: u8) -> Vec<u8> {
        vec![0, 0x90, key, 100, 96, 0x80, key, 17, 0, 0xff, 0x2f, 0]
    }
    ...
    fn sep() -> Vec<u8> {
        ...
        for (id, key) in [(4_u16, 60), (9_u16, 72)] {
            let events = score(key);
            bytes.extend(id.to_be_bytes());
            bytes.extend([0, 96, 7, 0xa1, 0x20, 4, 2]);
    ...
    fn converted_seq(key: u8) -> Vec<u8> {
        ...
        let mut b = (size as u32).to_le_bytes().to_vec();
        b.extend(500000_u32.to_le_bytes());
        b.extend([96, 0, 4, 2]);
        b.extend(events);
        b.resize(size, 0);
        b
    ...
    fn compatibility_song_selection_is_explicit_and_never_first_entry_fallback() {
        let bytes = sep();
        let c = catalog_source(&bytes, None).unwrap();
        assert_eq!(c.profile, Some(SourceProfile::SonySepV0));
        assert_eq!(c.songs.iter().map(|s| s.id).collect::<Vec<_>>(), [4, 9]);
        assert!(
            decode_source(&bytes, &Settings::default())
                .unwrap_err()
                .contains("explicitly")
        );
        let mut settings = Settings {
            source_selection: Some(
                select_source(&bytes, SourceProfile::SonySepV0, Some(9), None).unwrap(),
            ),
            ..Default::default()
        };
    ...
    fn converted_reimport_rejects_changed_or_reordered_selection_until_reselected() {
        ...
        let mut settings = Settings {
            source_selection: Some(
                select_source(&bytes, SourceProfile::ConvertedSeqLe32V1, None, Some(1)).unwrap(),
            ),
            ..Default::default()
        };
        assert_eq!(
            decode_source(&bytes, &settings).unwrap().source_events[0].data[0],
            72
        );
        let mut changed = converted_seq(72);
        changed.extend(converted_seq(60));
        assert!(
            decode_source(&changed, &settings)
                .unwrap_err()
                .contains("reselect")
        );
        settings.source_selection = Some(
            select_source(&changed, SourceProfile::ConvertedSeqLe32V1, None, Some(0)).unwrap(),
        );
        assert_eq!(
            decode_source(&changed, &settings).unwrap().source_events[0].data[0],
            72
        );
        assert!(
            decode_source(&changed[..changed.len() - 1], &settings).is_err(),
            "unselected songs must also be valid"
        );
    }
    #[test]
    fn complete_source_ledger_roundtrips_and_hard_blockers_ignore_no_midi_toggle() {
        let bytes = seq(&[
            0, 0xb0, 0, 3, 0, 0xa0, 60, 64, 0, 0x80, 60, 31, 0, 0xff, 0x51, 7, 0xa1, 0x20, 0, 0x2f,
            0,
        ]);
        let mut settings = Settings {
            source_selection: Some(
                select_source(&bytes, SourceProfile::SonySeqV1, Some(0), None).unwrap(),
            ),
        ...
        assert_eq!(ir.source_events[0].data, [0, 3]);
        assert_eq!(ir.source_events[2].data, [60, 31]);
        assert_eq!(ir.source_events[3].data, [0x51, 7, 0xa1, 0x20]);
        assert!(!ir.source_events[4].explicit_status);
        assert_eq!(ir.source_events[4].status, 0xff);
        let encoded = serde_json::to_vec(&ir.source_events).unwrap();
        let restored: Vec<crate::sequence_ir::SourceEvent> =
        ...
        settings.ignore_unsupported = false;
        assert!(settings.validate_playback(&ir).is_err());
    }
    #[test]
    fn compatibility_channel_ten_is_not_silently_general_midi() {
        ...
        assert!(
            settings
                .validate_playback(&decode_source(&bytes, &settings).unwrap())
                .unwrap_err()
                .contains("channel 10")
        );
    }
    ...
    fn converted_profile_alias_migrates_without_changing_selection_or_psx_payload() {
        ...
        assert_eq!(
            SourceProfile::from_id(legacy_id).unwrap().id(),
            "converted-seq-le32-v1"
        );
        for profile in [
            SourceProfile::SonySeqV1,
            SourceProfile::SonySepV0,
        ...
        assert_eq!(
            crate::psx_sequence::sequence_payload(&ir, &restored, bank)
                .unwrap()
                .1,
            crate::psx_sequence::sequence_payload(
                &decode_source(&bytes, &settings).unwrap(),
                &settings,
                bank
            )
            .unwrap()
        ...
        assert!(
            decode_source(&reordered, &restored)
                .unwrap_err()
                .contains("reselect")
        );
    }
    ...
    fn legacy_settings_json_and_future_selection_fields_are_preserved() {
        ...
        assert!(value.get("source_selection").is_none());
        let before: Settings = serde_json::from_value(value.clone()).unwrap();
        assert_eq!(serde_json::to_value(before).unwrap(), value);
        ...
        future.source_selection.as_mut().unwrap().schema_version = 99;
...
mod tests {
    ...
    fn midi_interpretation_migration_preserves_old_settings_until_explicit_upgrade() {
        ...
        std::fs::create_dir_all(root.join("assets")).unwrap();
        let events = [
            0, 0xb0, 101, 0, 0, 100, 0, 0, 6, 12, 0, 0x90, 60, 100, 96, 0x80, 60, 0, 0, 0xff, 0x2f,
            0,
        ];
        ...
        assert!(ir.events.iter().any(|e| matches!(
            e.kind,
            crate::sequence_ir::EventKind::Parameter {
                channel: 0,
                parameter: 0,
                value: 1200
            }
        )));
        let actual = assets::commit(
            prepare(
                &root,
                "assets/rpn.mid",
                "assets/rpn.epokasset",
        ...
        assert_eq!(
            package.meta.settings.sequence().unwrap().extra["future_settings"],
        ...
        let error = settings
            .validate_playback(&decode_source(&unknown, &settings).unwrap())
            .unwrap_err();
        assert!(
            error.contains("CC 119=0")
                && error.contains("track 1, tick 0")
                && error.contains("Legacy v1 exception")
        );
        let legacy = Settings {
            midi_profile: crate::midi::MidiProfile::LegacyV1,
            ..settings
        };
        legacy
            .validate_playback(&decode_source(&unknown, &legacy).unwrap())
            .unwrap();
    }
