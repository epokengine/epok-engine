            ...
            if let Some(id) = m.selected {
                match m.index.resolve(id).cloned() {
                    Ok(record) => {
                        if matches!(
                            record.meta.kind,
                            assets::Kind::MusicSequence | assets::Kind::SoundBank
                        ) {
                            ui.text_wrapped(assets::path_string(&m.root, &record.path));
                            ...
                            if ui.button("Change sequence / bank settings...") {
                        ...
                        ui.modal_popup_config("Delete asset?").always_auto_resize(true).build(||{
                                ...
                                if ui.button("Move to trash"){
                                    let in_use=scene.actors.iter().any(|entity|entity.audio.as_ref().is_some_and(|a|a.clip==Some(id)));
...
fn import_dialog(ui: &Ui, m: &mut Manager) {
    if m.form
        .as_ref()
        .is_some_and(|f| f.sequence.is_some() || f.bank.is_some())
    {
...
pub fn bank_selector(
    ui: &Ui,
    label: &str,
    selected: &mut Option<uuid::Uuid>,
    index: &assets::Index,
    default_label: &str,
) {
    ...
    if let Some(_combo) = ui.begin_combo(label, title) {
        if ui
            .selectable_config(default_label)
            ...
            .filter(|r| r.meta.kind == assets::Kind::SoundBank)
        {
            if ui
                .selectable_config(format!(
                    "{}##{}",
                    r.path.file_stem().unwrap_or_default().to_string_lossy(),
                    r.meta.id
                ))
                .selected(*selected == Some(r.meta.id))
                .build()
            {
                *selected = Some(r.meta.id);
            }
            #[cfg(test)]
...
fn portable_audio_dialog(ui: &Ui, m: &mut Manager) {
    ...
    if install_reference {
        m.start_reference_bank();
    }
    if start {
        m.start_import();
    }
    if !open && !m.busy {
        m.music_conversion.cancel();
        m.form = None;
    }
}
...
fn midi_byte(ui: &Ui, label: &str, value: &mut u8, min: i32) {
    ...
    if ui.input_int(label, &mut edit).build() {
        *value = edit.clamp(min, 127) as u8;
    }
}
fn bank_editor(ui: &Ui, bank: &mut crate::sound_bank::Settings, index: &assets::Index) {
    ...
    ui.input_text_multiline("Provenance / license", &mut bank.provenance, [-1., 58.])
        .build();
    ...
    for (pi, program) in bank.programs.iter_mut().enumerate() {
        ...
        if !ui.collapsing_header(
            format!(
                "Program {} / {:?}##mapping",
                program.program, program.drum_key
            ),
        ...
        if ui.checkbox("Channel 10 percussion mapping", &mut drums) {
            program.drum_key = drums.then_some(36);
        }
        if let Some(key) = &mut program.drum_key {
            midi_byte(ui, "Drum key", key, 0);
        }
        if ui.small_button("Remove mapping") {
            remove_program = Some(pi);
        }
        ...
        for (zi, zone) in program.zones.iter_mut().enumerate() {
            ...
            let name = index
                ...
                .unwrap_or_else(|_| "Choose an AudioClip".into());
            if let Some(_combo) = ui.begin_combo("Sample", name) {
                for r in index
                    .usable()
                    .filter(|r| r.meta.kind == assets::Kind::AudioClip)
                {
                    if ui.selectable(format!(
                        ...
                        zone.sample = r.meta.id;
                    }
                }
            }
            midi_byte(ui, "Lowest key", &mut zone.key_range[0], 0);
            midi_byte(ui, "Highest key", &mut zone.key_range[1], 0);
            midi_byte(ui, "Lowest velocity", &mut zone.velocity_range[0], 1);
            midi_byte(ui, "Highest velocity", &mut zone.velocity_range[1], 1);
            midi_byte(ui, "Root key", &mut zone.root_key, 0);
            crate::gui::Drag::new("Fine tune (cents)")
                .range(-100., 100.)
                .speed(0.1)
                .build(ui, &mut zone.fine_tune_cents);
            crate::gui::Drag::new("Gain")
                .range(0., 4.)
                .speed(0.01)
                .build(ui, &mut zone.gain);
            crate::gui::Drag::new("Pan")
                .range(-1., 1.)
                .speed(0.01)
                .build(ui, &mut zone.pan);
            for (label, value) in [
                ("Attack (ms)", &mut zone.envelope.attack_ms),
                ("Decay (ms)", &mut zone.envelope.decay_ms),
                ("Release (ms)", &mut zone.envelope.release_ms),
            ] {
            ...
            crate::gui::Drag::new("Sustain level")
                .range(0., 1.)
                .speed(0.01)
                .build(ui, &mut zone.envelope.sustain);
            let mut looping = zone.sample_loop.is_some();
            if ui.checkbox("Sample loop (original source frames)", &mut looping) {
                zone.sample_loop = looping.then_some([0, 28]);
            }
            if let Some(region) = &mut zone.sample_loop {
                let mut edit = [region[0] as i32, region[1] as i32];
                if ui.input_int2("Start / exclusive end", &mut edit).build() {
                    *region = [edit[0].max(0) as u32, edit[1].max(0) as u32];
                }
            }
            if ui.small_button("Remove zone") {
                remove_zone = Some(zi);
            }
        }
        if let Some(i) = remove_zone {
            program.zones.remove(i);
        }
        if ui.small_button("Add zone") {
            program
                .zones
                .push(crate::sound_bank::Zone::new(uuid::Uuid::nil()));
        }
    }
    if let Some(i) = remove_program {
        bank.programs.remove(i);
    }
        ...
        let program = (0..=127)
            .find(|p| {
                !bank
                    .programs
                    .iter()
                    .any(|b| b.program == *p && b.drum_key.is_none())
            })
            .unwrap_or(0);
        bank.programs.push(crate::sound_bank::Program {
            program,
            drum_key: None,
            zones: vec![crate::sound_bank::Zone::new(uuid::Uuid::nil())],
            extra: Default::default(),
        });
    }
    if let Err(error) = bank.validate() {
...
pub fn component(ui: &Ui, e: &Editor, entity: &mut crate::scene::Actor) {
    ...
    if let Some(_combo) = ui.begin_combo(crate::gui::field(ui, "Playable Audio"), label) {
        ...
        for record in e.assets.index.usable().filter(|r| {
            matches!(
                r.meta.kind,
...
pub mod interaction {
    ...
    pub fn verify_sequence(context: &mut imgui::Context) {
        ...
        let pending = crate::asset_manager::Pending {
            source: source.path.clone(),
            hash: source.hash.clone(),
            existing: None,
            status: Status::Pending,
        ...
        click(context, &mut e, "bank-selector");
        frame(context, &mut e);
        ...
        assert!(
            e.assets.busy,
            "Actual Import button must start the sequence transaction"
        );
        ...
        assert_eq!(
            package.meta.settings.sequence().unwrap().sound_bank,
            Some(bank)
        );
    ...
    fn soundfont_library_import_dialog_preserves_one_authoritative_bank() {
        ...
        e.assets.begin_pending(&pending);
        fn frame(ctx: &mut imgui::Context, editor: &mut Editor) {
            windows(ctx.frame(), editor);
            ctx.render();
        }
        ...
        assert_eq!(
            index.usable().count(),
            1,
            "embedded samples never become AudioClip assets"
        );
        ...
        assert_eq!(
            e.assets.form.as_ref().unwrap().bank.as_ref().unwrap(),
            package.meta.settings.sound_bank().unwrap()
        );
        assert!(
            !root.join(".epok/imported").exists(),
            "authoring import must not cook PSX samples"
        );
    }
    ...
    fn sony_sep_import_dialog_requires_and_persists_explicit_song() {
        ...
        assert_eq!(
            crate::sequence::catalog_source(&overlapping_prefix, None)
                .unwrap()
                .profile,
            Some(crate::sequence::SourceProfile::SonySepV0)
        );
        assert!(
            crate::sequence::catalog_source(
                &overlapping_prefix,
                Some(crate::sequence::SourceProfile::SonySeqV1)
            )
            .is_err()
        );
        assert_eq!(
        ...
        fn click(ctx: &mut imgui::Context, editor: &mut Editor, key: &str) {
            let position = BUTTONS.with(|buttons| {
                *buttons
                    .borrow()
                    .get(key)
                    .unwrap_or_else(|| panic!("Visible control {key} was not rendered"))
            });
            ctx.io_mut().add_mouse_pos_event(position);
            frame(ctx, editor);
            ctx.io_mut()
        ...
        click(&mut context, &mut e, "bank-selector");
        frame(&mut context, &mut e);
        ...
        assert!(
            e.assets.busy,
            "Real Import button must start the validating worker"
        );
        settle(&mut context, &mut e);
        assert!(
            !destination.exists(),
            "Unselected SEP must not publish an authoritative asset"
        );
        ...
        assert!(
            e.assets
                .error
                .as_deref()
                .is_some_and(|error| error.contains("explicitly")),
            "{:?}",
            e.assets.error
        );
        ...
        click(&mut context, &mut e, "source-song-combo");
        frame(&mut context, &mut e);
        ...
        assert_eq!(
            ...
            "Both SEP songs, headers and bytes must survive import"
        );
        ...
        assert_eq!(
            crate::sequence::decode_source(&package.source, settings)
                .unwrap()
                .source_events[0]
                .data,
            [72, 100]
        ...
        assert_eq!(
            crate::sequence::catalog_source(&package.source, None)
                .unwrap()
                .songs
                .len(),
            2
        ...
        assert!(
            !root.join(".epok/imported").exists(),
            "Inspection/import must not cook a target"
        );
    }
}
