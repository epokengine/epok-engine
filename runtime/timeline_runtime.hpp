...
template<size_t Capacity=8> class Director {
    ...
    static bool same_property(const Instance& instance,size_t a,size_t b){
        ...
        return p.id==q.id&&instance.targets[p.slot].same(instance.targets[q.slot]);
    }
        ...
        for(uint16_t i=0;i<asset.property_count;++i){
            ...
            for(uint16_t j=i;j<asset.property_count;++j){
                ...
                for(uint8_t lane=0;lane<property.channels;++lane){
                    const auto sampled=sample(property.curves[lane],instance.tick);
    ...
    void finish(Instance& instance,State state){
        ...
        for(uint16_t i=0;i<instance.asset->property_count;++i){
...
public:
    ...
    Handle play(const Asset& asset,BoundTarget owner,const BoundTarget* targets,uint32_t scene){
        ...
        for(uint16_t i=0;i<asset.property_count;++i){
            const auto& p=asset.properties[i];if(p.slot>=asset.target_count||!p.channels||p.channels>3||!p.curves){add(stats.dropped);return {};}
        }
        ...
        for(const auto& instance:instances)if(instance.state==State::Playing){
            for(uint16_t i=0;i<asset.property_count;++i)for(uint16_t j=0;j<instance.asset->property_count;++j){
                ...
                if(a.id==b.id&&targets[a.slot].valid()&&targets[a.slot].same(instance.targets[b.slot])){
    ...
    bool seek(Handle h,int32_t tick,uint32_t scene){
        auto* instance=find(h);if(!instance||instance->busy||!alive(*instance,scene)||tick<instance->tick)return false;
        ...
        while(instance->cursor<instance->asset->signal_count&&instance->asset->signals[instance->cursor].tick<=instance->tick){
            const auto signal=instance->asset->signals[instance->cursor++];
            if(signal.event&&instance->asset->events[signal.index].idempotent)invoke(*instance,signal.index);
