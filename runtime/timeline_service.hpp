...
inline bool stop(EntityHandle owner){auto* value=component(owner);if(!value)return false;const bool stopped=sequences.stop(value->playback);publish_stats();return stopped;}
...
inline void remove_owner(EntityHandle owner){sequences.cancel_owner(owner);publish_stats();}
