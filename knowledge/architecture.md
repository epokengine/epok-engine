...
| `lua_asset`, `lua_frontend`, `script_ir`, `lua_compile`, `lua_aot`, `lua_vm`, `lua_bytecode`, `lua_dependencies` | Lua authoring provider: discovery, static declarations, the `epok-lua` profile frontend, the shared typed IR, registry publication and the native/VM code generation and packaging |
