...
The editor version advances, so a project descriptor must read `editor_version: 0.4.0`. A project authored with v0.3.0 moves forward through the Hub's confirmed upgrade, which backs the descriptor up into `.epok/migrations` before publishing a new one; headless open paths reject a mismatched revision rather than migrating silently. The shipped `examples/sample-game` and both `examples/gameplay-api-parity` projects are updated. `examples/rpg-2-5d-demo` and `examples/timeline-spell` deliberately remain at `0.1.0`: their startup maps still use the retired Entity/Behaviour format, so the upgrade declines to publish a descriptor whose startup scene cannot load, and those two need the actor migration first.
...
- The forbidden-terms sweep runs as an ordinary test over every tracked file, so the product-naming rule no longer depends on review.
